...
export default function XLSFormWorksheet(props: Props) {
        ...
        return (rowIndex: number, columnIndex: number) => {
            ...
            if (value !== undefined && typeof value !== 'string') {
                if (localizableColumnNames.includes(key)) {
                    ...
                    return <Cell><Code>{JSON.stringify(value)}</Code></Cell>
                }
            }
    ...
    return <OverflowScrollContainer style={{ ...props.style }}>
        <FlexTable numRows={numRows} enableRowResizing={false}>
            {columnNames.map(columnName => <Column name={columnName} cellRenderer={cellRenderer} />)}
        </FlexTable>
