import loadFormFromXLSXFile, { nestDoubleColonFields } from './loadSurveyFromXLSX';

describe('loadSurveyFromXLSX()', () => {
  it('loads a complex survey from a exported XLSX', async () => {
    await loadFormFromXLSXFile('src/xlsform-simple-schema/test-data/wheelmap-survey.xlsx');
  });
});
...
describe('nestDoubleColonFields()', () => {
  it('Replaces non-suffixed properties with suffixed ones', () => {
    const nestedRecord = nestDoubleColonFields(
      { label: 'A label in default language', 'label::Deutsch (de)': 'Ein Feldtitel auf Deutsch' },
      ['label'],
      'English (en)'
    );
