import { SemanticError, FunctionNotImplementedError } from '../../../types/Errors';
...
export type FormulaFunction = (
  this: { context: ODKFormulaExecutionContext; scope: ODKNodeWithoutRuntimeInfo },
  ...args: Value[]
) => Value;

function assertString(value: unknown): asserts value is string {
  if (typeof value !== 'string') {
    throw new SemanticError(`Expected a string value as parameter, but got ${value}`);
  }
}

function assertNumber(value: unknown): asserts value is number {
  if (typeof value !== 'number') {
    throw new SemanticError(`Expected a number value as parameter, but got ${value}`);
  }
}

function assertBoolean(value: unknown): asserts value is boolean {
  if (typeof value !== 'boolean') {
    throw new SemanticError(`Expected a boolean value as parameter, but got ${value}`);
  }
}
...
export const functions: Record<string, FormulaFunction> = {
  ...
  */
  ...
  selected(space_delimited_array: Value, string: Value): boolean {
    assertString(space_delimited_array);
    assertString(string);
